...
using Kotocorn.Modules.Music.Common.Exceptions;
using Kotocorn.Modules.Music.Common.SongResolver;
using Kotocorn.Common.Collections;
using System;
using Microsoft.EntityFrameworkCore;

namespace Kotocorn.Modules.Music.Services
{
    public class MusicService : INService, IUnloadableService
    {
        ...
        public MusicService(DiscordSocketClient client, IGoogleApiService google,
            KotocornStrings strings, ILocalization localization, DbService db,
            SoundCloudApiService sc, IBotCredentials creds, Kotocorn bot)
        {
            _client = client;
            _google = google;
            _strings = strings;
        ...
        public async Task TryQueueRelatedSongAsync(SongInfo song, ITextChannel txtCh, IVoiceChannel vch)
        {
            ...
            var si = await ResolveSong(related[new KotocornRandom().Next(related.Length)], _client.CurrentUser.ToString(), MusicType.YouTube);
            if (si == null)
