...
using Kotocorn.Core.Modules.Gambling.Common.Events;
using System.Collections.Concurrent;
using Kotocorn.Modules.Gambling.Common;
using Discord;
...
namespace Kotocorn.Modules.Gambling.Services
{
            ...
            try
            {
                using (var req = new HttpRequestMessage(HttpMethod.Get, _creds.VotesUrl))
                {
                    ...
                    var ids = JsonConvert.DeserializeObject<VoteModel[]>(resStr)
                        .Select(x => x.User)
                        .Distinct();
                    await _cs.AddBulkAsync(ids, ids.Select(x => "Voted - <https://discordbots.org/bot/Kotocorn/vote>"), ids.Select(x => 10L), true);
                }
