...
using Kotocorn.Modules.NSFW.Exceptions;
using Kotocorn.Modules.Searches.Exceptions;

namespace Kotocorn.Modules.NSFW
{
    // thanks to halitalf for adding autoboob and autobutt features :D
    public class NSFW : KotocornTopLevelModule<SearchesService>
    {
        ...
        private async Task InternalBoobs(IMessageChannel Channel)
        {
            ...
            {
                JToken obj;
                obj = JArray.Parse(await _service.Http.GetStringAsync($"http://api.oboobs.ru/boobs/{new KotocornRandom().Next(0, 10330)}").ConfigureAwait(false))[0];
        ...
        private async Task InternalButts(IMessageChannel Channel)
        {
            ...
            {
                JToken obj;
                obj = JArray.Parse(await _service.Http.GetStringAsync($"http://api.obutts.ru/butts/{new KotocornRandom().Next(0, 4335)}").ConfigureAwait(false))[0];
            ...
            t = new Timer(async (state) =>
            {
                try
                {
                    if (tagsArr == null || tagsArr.Length == 0)
                        ...
                        await InternalHentai(Context.Channel, tagsArr[new KotocornRandom().Next(0, tagsArr.Length)], true).ConfigureAwait(false);
                }
                catch
                {
                    // ignored
