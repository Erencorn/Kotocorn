...
using Kotocorn.Modules.CustomReactions.Extensions;
using Kotocorn.Modules.Permissions.Common;
using Kotocorn.Modules.Permissions.Services;
using Kotocorn.Core.Services.Impl;
using Newtonsoft.Json;

namespace Kotocorn.Modules.CustomReactions.Services
{
    public class CustomReactionsService : IEarlyBlockingExecutor, INService
    {
        ...
        private readonly KotocornStrings _strings;
        private readonly IDataCache _cache;
        private readonly GlobalPermissionService _gperm;

        public CustomReactionsService(PermissionService perms, DbService db, KotocornStrings strings,
            DiscordSocketClient client, CommandHandler cmd, IBotConfigProvider bc, IUnitOfWork uow,
            IDataCache cache, GlobalPermissionService gperm, Kotocorn bot)
        {
        ...
        public CustomReaction TryGetCustomReaction(IUserMessage umsg)
        {
            ...
            if (GuildReactions.TryGetValue(channel.Guild.Id, out CustomReaction[] reactions))
                if (reactions != null && reactions.Any())
                {
                    ...
                    if (rs.Length != 0)
                    {
                        var reaction = rs[new KotocornRandom().Next(0, rs.Length)];
                        if (reaction != null)
                        {
            ...
            if (grs.Length == 0)
                return null;
            var greaction = grs[new KotocornRandom().Next(0, grs.Length)];
