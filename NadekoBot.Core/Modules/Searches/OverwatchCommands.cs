...
namespace Kotocorn.Modules.Searches
{
    ...
    {
        [Group]
        public class OverwatchCommands : KotocornSubmodule
        {
            public enum Region
            {
                Eu,
            ...
            public async Task<OverwatchApiModel.OverwatchPlayer> GetProfile(Region region, string battletag)
            {
                try
                {
                    using (var handler = new HttpClientHandler())
                    {
                        ...
                        {
                            http.AddFakeHeaders();
                            var url = $"https://owapi.Kotocorn.me/api/v3/u/{battletag}/stats";
                            var res = await http.GetStringAsync($"https://owapi.Kotocorn.me/api/v3/u/{battletag}/stats");
