...
using Kotocorn.Modules.Searches.Common.Exceptions;
using Newtonsoft.Json;
using NLog;

namespace Kotocorn.Modules.Searches.Services
{
    public class StreamNotificationService : INService
    {
        ...
        public StreamNotificationService(Kotocorn bot, DbService db, DiscordSocketClient client,
            KotocornStrings strings, IDataCache cache, IBotCredentials creds)
        {
            _db = db;
            _client = client;
            _strings = strings;
        ...
        public EmbedBuilder GetEmbed(FollowedStream fs, IStreamResponse status, ulong guildId)
        {
            var embed = new EmbedBuilder()
                ...
                .AddField(efb => efb.WithName(GetText(fs, "viewers"))
                                ...
                                .WithIsInline(true))
                .WithColor(status.Live ? Kotocorn.OkColor : Kotocorn.ErrorColor);
