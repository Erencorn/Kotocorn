...
using Kotocorn.Modules.Pokemon.Common;
using Kotocorn.Modules.Pokemon.Services;

namespace Kotocorn.Modules.Pokemon
{
    public class Pokemon : KotocornTopLevelModule<PokemonService>
    {
            ...
            if (targetType == null)
            {
                await Context.Channel.EmbedAsync(_service.PokemonTypes.Aggregate(new EmbedBuilder().WithDescription("List of the available types:"),
                        (eb, pt) => eb.AddField(efb => efb.WithName(pt.Name)
                                                          .WithValue(pt.Icon)
                                                          .WithIsInline(true)))
                            .WithColor(Kotocorn.OkColor)).ConfigureAwait(false);
                return;
            }
