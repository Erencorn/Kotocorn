...
namespace Kotocorn.Modules.Permissions
{
    ...
    {
        [Group]
        public class CommandCostCommands : KotocornSubmodule
        {
            ...
            {
                //_commandCosts = new ConcurrentDictionary<string, int>(Kotocorn.BotConfig.CommandCosts.ToDictionary(
            ...
            //    else
            //        await Context.Channel.SendConfirmAsync($"{Format.Bold(cmd.Name)} now costs {cost}{Kotocorn.BotConfig.CurrencySign} to run.").ConfigureAwait(false);
            //}
        }
    }
}
