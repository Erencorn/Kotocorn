...
using Kotocorn.Modules.Help.Services;
using Kotocorn.Modules.Permissions.Services;
using Kotocorn.Common;
using Kotocorn.Common.Replacements;
using Newtonsoft.Json;
using Kotocorn.Core.Common;
using Kotocorn.Core.Modules.Help.Common;

namespace Kotocorn.Modules.Help
{
    public class Help : KotocornTopLevelModule<HelpService>
    {
        public const string PatreonUrl = "https://patreon.com/kotocorn";
        public const string PaypalUrl = "https://paypal.me/erenx447";
        ...
        [KotocornCommand, Usage, Description, Aliases]
        [KotocornOptions(typeof(CommandsOptions))]
        ...
        public async Task Hgit()
        {
            ...
            foreach (var com in _cmds.Commands.OrderBy(com => com.Module.GetTopLevelModule().Name).GroupBy(c => c.Aliases.First()).Select(g => g.First()))
            {
                ...
                var opt = ((KotocornOptions)com.Attributes.FirstOrDefault(x => x is KotocornOptions))?.OptionType;
                if (opt != null)
                {
        ...
        public async Task Guide()
        {
            await ConfirmLocalized("guide",
                "https://Kotocorn.me/commands",
                "http://Kotocorn.readthedocs.io/en/latest/").ConfigureAwait(false);
        }
