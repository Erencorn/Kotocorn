...
namespace Kotocorn.Core.Services.Database
{
    public class KotocornContextFactory : IDesignTimeDbContextFactory<KotocornContext>
    {        
        public KotocornContext CreateDbContext(string[] args)
        {
            var optionsBuilder = new DbContextOptionsBuilder<KotocornContext>();
            var builder = new SqliteConnectionStringBuilder("Data Source=data/Kotocorn.db");
            builder.DataSource = Path.Combine(AppContext.BaseDirectory, builder.DataSource);
            optionsBuilder.UseSqlite(builder.ToString());
            var ctx = new KotocornContext(optionsBuilder.Options);
            ctx.Database.SetCommandTimeout(60);
            return ctx;
        }
    }

    public class KotocornContext : DbContext
    {
        ...
        public KotocornContext(DbContextOptions<KotocornContext> options) : base(options)
        {
        }
