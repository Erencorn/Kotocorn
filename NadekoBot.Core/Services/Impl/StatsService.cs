...
namespace Kotocorn.Core.Services.Impl
{
    public class StatsService : IStatsService
    {
        ...
        public const string BotVersion = "2.20.0";
        public string Author => "Erencorn#9712";
        ...
        public StatsService(DiscordSocketClient client, CommandHandler cmdHandler,
            IBotCredentials creds, Kotocorn Kotocorn,
            IDataCache cache)
        {
                    ...
                    try
                    {
                        using (var http = new HttpClient())
                        {
                            using (var content = new FormUrlEncodedContent(
                                ...
                                { "servercount", Kotocorn.GuildCount.ToString() },
                    ...
                    using (var http = new HttpClient())
                    {
                        using (var content = new FormUrlEncodedContent(
                            new Dictionary<string, string> {
                                    ...
                                    { "guildCount", Kotocorn.GuildCount.ToString() },
                                    { "version", BotVersion },
                                    { "platform", platform }}))
                        {
                            content.Headers.Clear();
                            ...
                            await http.PostAsync("https://selfstats.Kotocorn.me/", content).ConfigureAwait(false);
                        }
                    }
                }
                catch
